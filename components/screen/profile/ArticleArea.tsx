...
const ArticleArea = forwardRef<ArticleAreaHandle, Props>(function ArticleArea({ onPressItem }, ref) {
        ...
        {items.map((item) => (
          ...
          >
            <View style={{ aspectRatio: 1, borderRadius: 4, overflow: 'hidden', backgroundColor: '#000' }}>
              {item.photoUrl ? (
                <Image source={{ uri: item.photoUrl }} style={StyleSheet.absoluteFillObject as any} resizeMode="cover" />
              ) : (
                <View style={[StyleSheet.absoluteFillObject as any, { backgroundColor: '#222' }]} />
              )}

              <View style={[StyleSheet.absoluteFillObject as any, { backgroundColor: 'rgba(0,0,0,0.40)' }]} />
              ...
              >
                <Text style={[styles.date, { fontSize: scaleFont(12) }]}>{dateLabel(item.createdAt)}</Text>

                <Text style={[styles.title, { fontSize: scaleFont(22), lineHeight: scaleFont(28) }]} numberOfLines={2}>
                  {item.title}
                </Text>
  ...
  return (
    <View style={{ gap: scaleHeight(4) }}>
      ...
      {loadingMore && <ActivityIndicator style={{ marginTop: scaleHeight(12), marginBottom: scaleHeight(8) }} />}
    </View>
  );
});
